...
export function AppBuilderGuide({ items, onOpenSkill, focus = null, onWorkflowChange }: AppBuilderGuideProps) {
  ...
  const skills = useMemo(() => {
    ...
    // Fallback: Das Privacy-Gate hält eigene Skills aus dem Public-Katalog heraus;
    // die Guide-Texte sind bereits sanitisiert und Teil des Bundles.
    for (const key of GUIDE_SKILL_KEYS) {
  ...
  return <section className="app-builder" aria-labelledby="app-builder-title">
    <header className="catalog-heading app-builder-heading">
      ...
      <div className="app-builder-principle"><Blocks aria-hidden="true" /><span><strong>Baukasten-Prinzip</strong><small>Eigenständig · austauschbar · erweiterbar</small></span></div>
    </header>
